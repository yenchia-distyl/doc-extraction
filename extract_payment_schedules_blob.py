...
class TextPaymentExtractor:
    ...
    def identify_line_of_business_fuzzy(self, content: str) -> str:
        """Use case and dash-insensitive search to identify line of business from content."""
        # Define the three possible lines of business with only core terms
        lob_keywords = {
            'MEDICAID': [
                'medicaid'
            ],
        ...
        # Split content into sections for weighted scoring
        content_length = len(content)
        beginning_section = content_lower[:min(2000, content_length // 4)]  # First quarter or 2000 chars
        ...
        for lob, keywords in lob_keywords.items():
            ...
            for keyword in keywords:
                # Count occurrences in entire document
                total_count = content_lower.count(keyword)
                
                # Count occurrences in beginning section (weighted more heavily)
                beginning_count = beginning_section.count(keyword)
                
                # Calculate weighted score
                # Beginning appearances get 3x weight, total appearances get 1x weight
                keyword_score = (beginning_count * 3) + total_count
                score += keyword_score
                
                if keyword_score > 0:
                    logger.info(f"Found '{keyword}' for {lob}: {total_count} total, {beginning_count} at beginning")
            
            scores[lob] = score
        
        # Find the line of business with highest score
        if not scores or max(scores.values()) == 0:
            logger.warning("No line of business keywords found in content")
            return 'UNKNOWN'
        
        best_lob = max(scores, key=scores.get)
        best_score = scores[best_lob]
        
        logger.info(f"Line of business scores: {scores}")
        logger.info(f"Identified line of business: {best_lob} (score: {best_score})")
        
        return best_lob
    
    async def extract_line_of_business_with_llm(self, session: aiohttp.ClientSession, 
                                                content: str) -> str:
        """Use fuzzy search first, fallback to LLM if needed."""
        # First try fuzzy search (faster and more reliable)
        fuzzy_result = self.identify_line_of_business_fuzzy(content)
        
        if fuzzy_result != 'UNKNOWN':
            logger.info(f"Fuzzy search identified line of business: {fuzzy_result}")
            return fuzzy_result
        
        # Fallback to LLM if fuzzy search fails
        logger.info("Fuzzy search failed, trying LLM fallback...")
